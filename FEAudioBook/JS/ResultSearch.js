...
async function fetchBooks(categoryId, rating) {
    try {
        const response = await fetch("http://localhost:8080/searchBook/" + localStorage.getItem("keywordToSearch")+ "/" + categoryId + "/" + rating);
        const books = await response.json();
        const sizeBook = document.getElementById("sizeBook");
        sizeBook.innerHTML =
            "Có tất cả: " + books.length + " cuốn sách cho từ khóa: " + localStorage.getItem("keywordToSearch");
